...
NDTmeter.prototype.onfinish = function (passed_results) {
    ...
    if (results_to_display.hasOwnProperty(metric_name)  &&
      ...
      if (metric_name !== 'MinRTT') {
        resultString = Number(passed_results[metric_name] /
            1000).toFixed(2);
        storage_entry[metric_name] = resultString;
        resultString += ' Mbps';
      } else {
  ...
  storage_entry['timestamp'] = Date.now();
  ...
  chrome.storage.local.get('storeResults', function(result) {
    if (result['storeResults'] === true) {
      chrome.storage.local.get('ndtResults', function(result) {
        result['ndtResults'].push(storage_entry);
  ...
  d3.selectAll("#progress-meter text.status").attr("dy", "-25px");
...
NDTmeter.prototype.reset_meter = function () {
  ...
  d3.selectAll('.result-value, .result-label').remove();
